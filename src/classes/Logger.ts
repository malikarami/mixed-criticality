...
export class Logger {
  ...
  utilization(data: { speed: number, U11: number; U22: number; U21: number; vdf: number; u: number; U12: number, necessaryCheck: {result: boolean, method: string}, sufficientCheck: {result: boolean, method: string}, taskSetCheck: boolean }) {
    this.stats.analysis = {
      ...this.stats.analysis,
      // @ts-ignore
      ...data,
      taskSetCheck: data.taskSetCheck,
      necessaryCheck: data.necessaryCheck.result,
      sufficientCheck: data.sufficientCheck.result,
    };
